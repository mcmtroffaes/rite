...
def test_string():
    ...
    assert x.fmap(repeat(str.capitalize)) == String('Hello')
...
def test_text():
    ...
    assert list(map(str.capitalize, x)) \
           == ['Hello', ' ', 'World']
    assert x.fmap(repeat(str.capitalize)) \
...
def test_protected():
    ...
    assert list(map(str.capitalize, x)) == ['Hello']
    assert x.fmap(repeat(str.capitalize)) == x
...
def test_text_combined():
    ...
    x2 = Text([String('HELLO '),
               Tag(TagType.STRONG, String('BRAVE')),
               Text([String(' WORLD')])])
    assert x1.fmap(repeat(str.upper)) == x2
...
def test_tag_combined():
    ...
    assert x11.fmap(repeat(str.upper)) == x12
    assert x21.fmap(repeat(str.upper)) == x22
